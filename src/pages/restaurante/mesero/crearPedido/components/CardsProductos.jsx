    ...
    return (
        <div className="w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {productos.map((product) => (
                <div key={product.id} onClick={() => { if (!hayPedido) agregarItem(product) }}
                ...
                >
                    <div className="flex items-center gap-4">
                        <div className="flex">
                            <img src={`${RouteBack}${product.imagen}`} alt={product.id} className="w-28 h-20 object-contain rounded-lg" />
                        </div>
