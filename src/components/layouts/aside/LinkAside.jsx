...
const LinkAside = React.memo(function LinkAside({ menu, activeSubroutes }) {
    ...
    return (
        <li>
            <NavLink to={menu.link} replace={true} className={`flex items-center p-2 text-gray-900 rounded-lg dark:text-white hover:bg-green-300 dark:hover:bg-green-400 group ${isActive() ? 'active' : ''}`}            >
                ...
                <span className="flex-1 text-lg ms-3 mt-1 whitespace-nowrap">{menu.texto}</span>
            </NavLink>
        </li>
    );
});
